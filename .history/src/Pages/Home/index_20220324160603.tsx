...
const Home = () => {
  ...
  const updateSections = (activeSection: any) => {
    ...
    if(activeSections.find(a => a === activeSection[0])){
      setActiveSections(activeSections.map(a => a !== activeSection[0]))

      setActiveSections(activeSections.map(a => {
        if (a !== activeSection[0]) {
          return a
        }
    ...
    if(!activeSections.find(a => a === activeSection[0])){
      setActiveSections([activeSection[0], ...activeSections])
    }
